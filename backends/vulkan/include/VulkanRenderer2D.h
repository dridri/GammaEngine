...
#ifndef VULKANRENDERER2D_H
#define VULKANRENDERER2D_H
...
class VulkanRenderer2D : public Renderer2D, public VulkanRenderer
{
public:
	VulkanRenderer2D( Instance* instance, uint32_t width, uint32_t height );
	~VulkanRenderer2D();
	...
	virtual void DrawText( int x, int y, GE::Font* font, uint32_t color, const std::string& text, TextFlags flags = Nil );
	virtual void DrawText( int x, int y, GE::Font* font, uint32_t color, const std::wstring& text, TextFlags flags = Nil );

	virtual void DrawLine( int x0, int y0, uint32_t color0, int x1, int y1, uint32_t color1 );

protected:
	void Compute();
	void Prerender( GE::Image* image );
